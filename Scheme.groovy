...
class FunctionStack {
    ...
    def cambiar(lista, nuevo, viejo){
       lista.collectNested { if (it == viejo){ it = nuevo  }else{it = it}  } 
    }
        ...
        if (!first){
            ...
            switch(variable.getParam()) {
                ...
                case 3:
                    ...
                    break
                case 0:
                    def analize1 = analize(parseList[1])
                    def para = cambiar(variable.parameter,''+ analize1,variable.invoke[1])
                    variable.setDefinitions(analize(para))   
                    break
